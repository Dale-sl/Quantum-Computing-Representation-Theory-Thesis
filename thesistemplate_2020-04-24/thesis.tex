...
% So are Acknowledgments (proper English spelling)
  ...
  The function will be nearly unknown for the first few queries, but after some asymptotically \todo{maybe this asymptotic claim is false}
  ...
  the group \cite[Proposition 1.10.1]{sagan} \todo{probably should at least write out the proposition here}. It is a fundemental abstact algebra fact that 
  ...
  \begin{prop}[{\cite[Pg. 88]{fulton}}]\label{prop1}
    The elements $v_T$, as $T$ varies over the \emph{standard} tableaux \todo{ define standard tableaux} of $\lambda$, form a basis for $S^\lambda$.
  \end{prop}
  \begin{proof}
...
\LComment{$tensorCharacter$ stores $V^{\otimes t}$, while $permutationCharcter$ stores $V$.}
...
\If{$probabiltyOfSuccess \geq \frac{2}{3}$ and $boundedQueries = 0$}
...
\EndProcedure
  ...
  \begin{proof}
    ...
    We observe that if
    \todo{this feels really ugly, the bijection seems too obvious to need this; also feels insufficent}
  ...
  \end{lemma}
  \begin{proof}
    We first recall that $\CC \Omega_k  \cong M^{(n-k,k)}$, and apply corollary $3.1.5$ \todo{maybe cleverref this}.
  ...
  \begin{proof}
    We observe that $V$ is the permutation representation of $S_n / S_{n-1}$ \todo{expand on this or prove it maybe}, so using the permutation representation case of lemma $3.1.9$ \todo{clever ref} we see that
    \begin{align*}
  ...
  Through this latter method we are able to compute the Kronecker coefficents for small partitions, although computing the Kronecker coefficents is in general $P\#$ complete.\todo{cite this  and formulas maybe}
  ...
  asymptotics of the logarithm of the dedekind numbers $\binom{n}{\lfloor{n/2} \rfloor}$\todo{maybe don't gain anything from saying this}.
...
\end{thm}
\begin{proof}
  By a restriction of Dvir's theorem \todo{cite and define this}, we know that
...
% We end with a weaker conjecture which, if proven, could lead to an inductive proof of $\Omega_k^{2k}$'s quantum query complexity when $2k$ is a power of $2$.
% \begin{conj}
...
% then for $2k/2^j \nin \ZZ$, the rows get shorter so that $\sum \lambda_i = 2k$.
% For example, if $k = 5$, then $S^{(5,5)} \otimes S^{(5,5)}$ contains the partition $(3^2,2^2)$ intead of $(3^4)$, and likwise $S^{(5,5)} \otimes (S^{(5,5)})^{\otimes 2}$ contains $(2^2,1^6)$ instead of $(2^8)$.
...
% That being said, $(\CC\Omega_k^{2k})^{\otimes (\ell - 1)}$
